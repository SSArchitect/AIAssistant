...
class AgentToolSkill(Skill):
    """Terminal tool wrapper around a specialized agent."""
    ...
    def _routing_guidance(self) -> str:
        ...
        if self.agent.id == "deep_research_v1":
            return "Use this agent for confirmed deep research workflows that need a research plan, searches, synthesis, and a report."
    ...
    def metadata(self) -> SkillMetadata:
        return SkillMetadata(
            name=self.agent.id,
            description=(
                f"Run the {self.agent.name} agent as a terminal tool. "
            ...
            tags=["agent", "terminal", self.agent.id],
            source="system",
        )
    ...
    async def execute(self, **kwargs) -> SkillResult:
        return SkillResult(
            success=False,
            error=f"{self.agent.id} is a terminal agent tool and must be handled by the agent engine.",
            data={"terminal": True, "agent_id": self.agent.id, "arguments": kwargs},
...
class DeepResearchAgentTool(AgentToolSkill):
    auto_discover = True

    def __init__(self):
        super().__init__(_required_agent("deep_research_v1"))
