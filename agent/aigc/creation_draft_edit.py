...
DRAFT_EDIT_GUIDANCE = '''
图片返工可重新生成（含image_layout局部构图），或编辑当前节点的待修草稿。候选已基本符合身份、环境与构图、仅有可局部修正的问题（如人物占比）时，优先考虑局部编辑，避免整张重画丢失正确内容。
...
def localized_repair_guidance(request):
    ...
    if (not node or node.get('kind') != 'image' or node.get('purpose') != 'shot_reference'
            or node['id'] in request.locked_node_ids or node.get('edit_source_asset_id') or node.get('image_layout')):
        return ''
...
def draft_edit_task(request):
    ...
    if not request.automatic_mode or not repair or not repair.findings:
        return None
    node = next((n for n in request.current_plan.get('nodes', []) if n['id'] == repair.node_id), None)
    if not node or node['id'] in request.locked_node_ids or node.get('kind') != 'image' or node.get('image_layout'):
        return None
