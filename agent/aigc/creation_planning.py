...
class RepairExecution(StrictModel):
    operation: Literal['generate', 'edit']
    edit_source_asset_id: str = ''
...
class RepairAttempt(StrictModel):
    ...
    execution: Optional[RepairExecution] = None
...
分镜同时有environment与composition时，工具仅从composition提取本轮明确请求且不与目标冲突的姿态、接触或机位关系，不上传该图的身份和背景像素。需要保持既有角色身份时必须单独绑定identity，不能暗中依赖composition传递人设。reference.note说明具体借用什么，不能把相反朝向或另一地点整体导入；需要站立御剑时优先选择本来就有双脚同一支撑物的清晰参考，不把手持兵器误当脚下载具。
