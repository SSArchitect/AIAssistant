...
- 同来源距上次普通提问达到 **30 分钟**，且没有排队或执行中的任务时，下一条普通消息自动建立新 Conversation。连续提问和未结束的任务继续沿用当前会话；计时按 Gateway 接收时间计算，控制指令、不支持的消息和重复投递不延长计时。人设、长期记忆及旧会话记录保留，旧短期上下文不进入新会话。计时持久化，重启不会重置；升级前的来源首次使用原来源更新时间作为计时基准。
