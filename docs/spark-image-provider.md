...
输入图上传使用独立的有界 120 秒网络窗口；图片提交预算按上传数量分配，仍受整次调用预算限制。不能只延长外层等待而保留 60 秒上传读写超时：生产中约 1.8 MB 场景图上传曾需 63 秒，可能在取得任务编号前反复失败。

图片服务把每张已确认的上传回执原子保存到 `data/spark-input-uploads`，仅记录内容指纹与 Provider 资产 ID，不保存图片、提示词或凭证。回执按服务地址、凭证指纹与原上传幂等键隔离。中途超时或进程重启后，已成功上传的图片直接复用，未确认上传仍使用原字节与原幂等键；图片次序和任务请求不变。相同键下输入变化会拒绝提交。过期回执仍可用于重放已受理任务；不得擅自换资产或新键以规避过期错误。损坏回执只能回放原上传，不能推测任务已经开始。
