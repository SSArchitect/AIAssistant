...
- 0.6.0 扩展支持 mode=image_to_video + 单首帧图片，详见 [图片输入接入](spark-image-inputs.md)。仍不支持参考视频、尾帧、负向提示词、模型选择或音频开关。宽高/帧数仅接受整数，FPS/时长接受数字；除 seed 外不接受数字字符串，数值字段均拒绝布尔值，未知字段报错。
...
图片和视频共用 `SparkTaskClient` 的提交、鉴权、重试、轮询和错误上下文逻辑。视频每 5 秒查询一次，`submitting`、`queued`、`running`、`recovering` 均继续等待，`failed` 和 `expired` 终止等待并返回错误。普通视频调用总等待期限为 30 分钟，包含下载；工具治理超时为 32 分钟，留出返回错误信息的余量。视频上传及提交阶段最多等待 15 分钟，单次 HTTP 读写超时为 5 分钟；流式聊天在获得 Provider task ID 后仍可追踪最多 6 小时。图片提交期限保持 60 秒。

同一账号、同一会话使用原幂等键继续调用时，执行层会从持久化 Trace 恢复 Provider task ID，直接 GET 原任务及产物，不再上传图片或 POST 新任务。尚无 task ID 时，可从原请求的带编号图片上下文恢复附件，并使用同一幂等键重试提交，不依赖追问消息重新携带附件；无法找到原图时返回明确的参数错误，不猜测图片。明确 404、过期和失败不会触发重新生成。没有获得 task ID 的超时表示提交结果未确认，不能宣称视频已在排队或生成。

视频调用通过权限校验后，首次准备好的完整参数会冻结在 Trace 数据库的私有 `video_requests` 表中，按账号、会话和幂等键隔离；重试忽略模型重新填写的提示词、参考图顺序、尺寸、时长及种子，使用首次参数。该表不进入 run/event API，账号清理同时删除其记录。历史任务没有冻结记录时，从最早一次调用恢复原图片顺序和非敏感参数，不采用失败重试的新顺序；历史提示词已脱敏，无法逐字恢复的部分仍由当前调用提供，Provider 的幂等校验继续防止覆盖已接受请求。
