...
验证状态（2026-09-20）：功能与两项实测修复已部署，配套 OTA 为 `0.4.1-ota.23`（sequence 36），26 个公网文件哈希及缓存检查通过。`./scripts/test.sh` 全量通过（Python 1300 项、前端 339 项，Go vet/test/build 全部通过；Android SDK 未配置，原生测试跳过）。新增回归覆盖完整执行稿字数预算、预留绑定空间、三图依次上传的时间窗口及未确认任务的有界等待。
