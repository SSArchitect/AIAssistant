...
- 会话记忆：`ConversationMemory` 保存单个 `conversation_id` 的短期上下文窗口。
- 角色记忆：`RoleMemoryStore` 按 `role_id` 隔离人设记忆 `persona` 和长期记忆 `long_term`，默认跨 Agent 共享，也支持未来按 `agent_id` 或 app scope 收窄。
- 记忆 hook：`HeuristicMemoryHook` 在对话结束后 review 当前 turn，只写入显式或高信号用户事实；后续可替换成 LLM-backed memory sub-agent。
...
响应会返回 `role_id`、本轮注入的 `memory_context` 和新增的 `memory_updates`，用于调试和后续 UI 展示。普通业务 UI 不应把隐藏记忆直接展示为 assistant 回复内容。
