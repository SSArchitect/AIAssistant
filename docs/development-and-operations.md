...
- 问答结果、研究报告、用户资料、笔记和后续需要检索复用的内容保存为 Drive 文件。
- 聊天回答的“保存到网盘”按钮会直接生成结构化 Markdown，包含问题、回答、来源、保存时间和会话/run 标识，默认保存到 `/知识库`，不会再发起第二次模型调用。
- 普通回答不会自动保存；只有用户明确要求“保存/归档到网盘”时，Super Chat 才会调用 `save_drive`。Deep Research 的报告归档仍属于其工作流步骤。
- 旧 `/api/projects` Knowledge 数据在 Gateway 启动时幂等迁移到 `/知识库/<旧项目>/`；旧表保留用于迁移兼容，但不再提供独立 Knowledge API。
- 当前 Drive 检索是文件名、摘要、标签和文本内容的关键词检索。后续向量索引应作为 Drive 文件之上的派生索引，而不是再建立一套平行知识存储。
...
- 当前只有 `super_chat` 暴露 Drive、Todo 和 Agent workflow 这类有产品状态的工具；其他 self runtime Agent 只获得与其边界相符的 enabled skills。
...
- `ls_drive`：列出目录。
- `search_drive`：搜索文件名、摘要、标签和文本内容。
- `read_drive`：读取文本文件。
- `save_drive`：新建文件；未指定目录时默认保存到 `/知识库`，可递归创建目录。
- `update_drive`：更新文件内容/元数据，或重命名、移动文件和文件夹。
- `mkdir_drive`：递归创建目录。
- `delete_drive`：永久删除文件或文件夹，默认策略 `confirm`。
- `share_drive`：开启/关闭文件公开分享链接，默认策略 `confirm`。
- `archive_url_to_drive`：读取公开网页并保存为 `/知识库/网页归档` 下的 Markdown，默认策略 `auto`。

Todo 内置工具：

- `create_todo`、`list_todos`、`get_todo`、`update_todo`。
- `delete_todo`：永久删除待办，默认策略 `confirm`。
...
- `delete_pulse_topic`：永久删除 Topic，默认策略 `confirm`。
...
模型可见的 Pulse 工具结果同样不返回 Topic ID。改名时使用 `original_name` 定位当前 Topic、用 `name` 提供新名称，内部 ID 只在授权准备与执行阶段解析；用户可见的方案和最终回答只展示 Topic 名称。
...
Pulse 和“今日聚焦”的自动预计算只覆盖最近 7 天有真实访问或使用的帐号：前台打开/恢复页面、真实输入操作、发送消息、登录和主动打开/操作 Pulse 卡片均计入。页面通过带帐号 session 的 `POST /api/accounts/activity` 记录服务器时间，按帐号每分钟限报一次；隐藏页面、任务轮询、模型消耗和自动曝光不更新活跃时间。连续 7 天无上述活动后，定时预计算、GET 自动补生成和“今日聚焦”补算均跳过；重新访问或使用后恢复。历史记录用用户消息、session 创建时间和主动 Pulse 事件补充，不使用会被轮询刷新的 session `last_used_at`。

正常成功间隔为 6 小时。自动任务全局串行，尝试时间和结果持久化在 `pulse_schedule_states`：首次失败退避 12 小时，连续失败最多退避 24 小时，因此 gateway 重启不会清空冷却并触发密集重试。用户显式点击刷新不受活跃期限和自动调度冷却限制。

管理员成本页提供“真实活跃与自动生成”表：展示最近真实活跃、最近用户消息、7 天用户消息数、自动生成资格及最近后台消耗。自动生成资格复用调度器同一查询，始终按当前最近 7 天判断；最近时间覆盖全部历史，后台调用与 Token 汇总则遵循成本日期筛选。原成本口径的“活跃用户”和“最近使用”在页面分别标为“有模型消耗的账号”和“最近模型消耗”，不再暗示真实访问。没有模型消耗的现存帐号也显示在活跃表中。
